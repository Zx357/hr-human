<template>
  <view class="template-advertise">
    ...
    <view class="adver-wrap tn-bg-white">
      <view class="tn-flex tn-flex-row-between tn-padding-top-lg" @tap="showLandscape">
        ...
        <view class="tn-flex-1 justify-content-item tn-padding tn-text-center">
          <view class="">
            <tn-icon name="qr-code" style="font-size: 70rpx;"></tn-icon>
          </view>
        ...
        <view class="tn-flex tn-flex-col-center tn-flex-row-left">
          ...
          <view class="tn-padding-right tn-color-black">
            <view class="tn-padding-right tn-padding-left-sm">
              <text class="tn-text-lg tn-text-bold">图鸟衣衣</text>
    ...
    <!-- 压屏窗-->
    <app-landscape :show="show2" @close="closeLandscape">
...
// 获取顶部销售信息容器相关信息
const getTopInfoRect = () => {
  // 使用uni.createSelectorQuery代替this._tGetRect
  const query = uni.createSelectorQuery().in(getCurrentInstance().proxy)
  query.select('#top-info').boundingClientRect((res) => {
    if (!res) {
      setTimeout(() => {
        getTopInfoRect()
      }, 50)
...
const handlePageScroll = (scrollTop) => {
  ...
  if (prevScrollTop.value > scrollTop) {
   // 内容下滑，即手指从下往上滚
   topInfoTranslateY.value = 0
  } else {
  // 内容上滑，即手指从上往下滚
    if (scrollTop > topInfo.height) {
      topInfoTranslateY.value = topInfo.height
    }
  }
