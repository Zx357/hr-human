  ...
  <view class="oa-content">
    ...
    <view class="" :style="{paddingTop: vuex_custom_bar_height + 30 + 'px'}" style="padding-bottom: 240rpx;">
      ...
      <view class="tn-bg-gray--light tn-padding tn-text-justify" style="border-radius: 10rpx;margin: 0 30rpx 30rpx 30rpx;">
        <textarea v-model="reason" maxlength="200" placeholder="请简单写一下设备事由" placeholder-style="color:#AAAAAA" style="height: 160rpx;width: 100%;"></textarea>
      </view>
...
const index = ref(99)
const array = ref(['个人设备', '部门设备', '公司设备', '其他用途'])
...
async function submitApply() {
  ...
  try {
    const res = await submitApplication({
      ...
      remark: `设备名称: ${deviceName.value.trim()}; 设备数量: ${deviceCount.value}`
    })
    uni.hideLoading()
    if (res.code === 200) {
      uni.showToast({ title: '提交成功', icon: 'success' })
      setTimeout(() => {
        uni.redirectTo({ url: '/workPages/prompt?result=success&title=设备申请' })
      }, 600)
    } else {
    ...
    setTimeout(() => {
      uni.redirectTo({ url: '/workPages/prompt?result=fail&title=设备申请' })
    }, 1200)
  } finally {
