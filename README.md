...
- **系统管理**:用户、角色、菜单(多语言)、字典(多语言)、操作日志审计
...
- **申请**:请假、加班、补卡、出差、换休、离职,提交后实时跟踪审批进度,待审批可撤销
...
- 安全基线:员工密码 BCrypt 加密存储(存量明文首次登录自动升级);JWT 与 Redis 会话绑定,登出/禁用/改密即时生效;管理端接口仅管理员角色可访问;跨域来源通过 `CORS_ORIGINS` 环境变量收敛;员工证件照文件名 UUID 化防枚举
- 定时任务:每日凌晨 2 点自动核算前一日考勤;到期提醒(合同/试用期/证书)见 `GET /reminder/list`
