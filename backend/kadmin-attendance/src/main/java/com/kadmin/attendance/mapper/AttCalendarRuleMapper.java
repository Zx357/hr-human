...
public interface AttCalendarRuleMapper extends BaseMapper<AttCalendarRule> {
    @Select("SELECT r.*, c.unit_name as company_name FROM att_calendar_rule r LEFT JOIN org_unit c ON r.company_id = c.id ORDER BY r.company_id, r.rule_type, r.start_date")
