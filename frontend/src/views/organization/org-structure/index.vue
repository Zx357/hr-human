...
const tableData = computed(() => {
  ...
  const mapItem = (item: { name: string; value: number }, i: number) => ({
    index: i + 1,
    label: item.name,
    count: item.value,
  ...
  switch (statsDimension.value) {
    ...
    case 'gender':
      return statistics.value.genderDistribution.map(mapItem);
    case 'education':
      return statistics.value.educationDistribution.map(mapItem);
    case 'status':
      return statistics.value.statusDistribution.map(mapItem);
    case 'employeeType':
      return statistics.value.employeeTypeDistribution.map(mapItem);
    default:
      return [];
  }
});

const dimensionLabel = computed(() => dimensionOptions.find(d => d.value === statsDimension.value)?.label || '');

const dimensionTitle = computed(() => {
  const prefix = selectedNode.value ? selectedNode.value.unitName : '全公司';
  return `${prefix} - ${dimensionLabel.value}`;
});
...
watch(statsDimension, () => nextTick(() => renderChart()));
...
function handleAdd(parent?: Api.Organization.OrgUnit) {
  ...
  editingData.value = {
    ...
    unitType: availableTypes[0].value,
    unitCode: '',
    unitName: '',
    shortName: '',
    leaderId: undefined,
...
function handleRefresh() {
  ...
  if (selectedNode.value) {
    loadStatistics(selectedNode.value.id);
  } else {
    loadStatistics();
  }
}
...
<template>
  <div class="org-page">
    <div class="org-left">
      <ElCard shadow="hover" class="full-h">
        <template #header>
          <div class="tree-header">
            <span class="tree-header-title">组织架构</span>
    ...
    <div class="org-right">
      <ElCard v-loading="statsLoading" shadow="hover" class="full-h">
        <template #header>
          <div class="stats-header">
            <div class="stats-title">
              <span class="stats-title-text">
                {{ selectedNode ? selectedNode.unitName : '全公司' }} - 人员结构分析
              </span>
              <ElTag
                v-if="selectedNode"
                :type="(getTypeTag(selectedNode.unitType).type as any)"
                size="small"
              >
        ...
        <ElRow :gutter="16">
            ...
            <ElCard shadow="never" class="stats-card">
              ...
              <div v-if="tableData.length > 0" class="stats-card-body">
                <ElTable :data="tableData" border stripe size="small">
                  ...
                  <ElTableColumn label="占比" width="140" align="center">
                    <template #default="{ row }">
                      <div class="percent-cell">
                        <ElProgress
                          :percentage="Number(row.percent)"
                          :stroke-width="8"
                          :show-text="false"
                        />
                ...
                <div class="stats-total">
                  合计：<span class="text-primary font-bold">{{ statistics.totalCount }}</span> 人
                </div>
              </div>
          ...
          <ElCol :span="12">
            <ElCard shadow="never" class="stats-card">
              ...
              <div v-if="tableData.length > 0" ref="chartRef" class="chart-container" />
    ...
    <ElDialog
      ...
      :title="operateType === 'add' ? '新增组织' : '编辑组织'"
      width="550px"
      destroy-on-close
    >
      <ElForm label-width="100px" :model="editingData">
        ...
        <ElFormItem label="编码" required>
          <ElInput
            ...
            placeholder="如：GRP001、COM001、DEPT001"
            :maxlength="50"
            show-word-limit
          />
        </ElFormItem>
