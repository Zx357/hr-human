...
const formData = ref<Application & { employeeName?: string; employeeNo?: string; companyName?: string; deptName?: string }>({
  employeeId: undefined as any, appType: 'reward', rewardType: 1
});
...
const searchParams = ref({ employeeName: '', employeeNo: '', status: undefined as number | undefined, appType: undefined as string | undefined });

async function loadDictData() {
  const [rewardRes, punishRes, genderRes, positionRes] = await Promise.all([
...
async function loadData() {
  ...
  try {
    ...
    if (!appType || appType === 'reward') {
      const res1 = await fetchApplicationPage({ pageNum: currentPage.value, pageSize: pageSize.value, appType: 'reward', employeeName: searchParams.value.employeeName || undefined, employeeNo: searchParams.value.employeeNo || undefined, status: searchParams.value.status });
      records = [...(res1.data?.records || [])];
      totalCount += res1.data?.total || 0;
    }
    if (!appType || appType === 'punish') {
      const res2 = await fetchApplicationPage({ pageNum: currentPage.value, pageSize: pageSize.value, appType: 'punish', employeeName: searchParams.value.employeeName || undefined, employeeNo: searchParams.value.employeeNo || undefined, status: searchParams.value.status });
...
watch(() => formData.value.rewardType, () => {
  formData.value.category = '';
});

function handleAdd() {
  formData.value = { employeeId: undefined as any, appType: 'reward', rewardType: 1, category: '', amount: 0, effectDate: '', reason: '', employeeName: '', employeeNo: '', companyName: '', deptName: '' };
...
function handleReset() { searchParams.value = { employeeName: '', employeeNo: '', status: undefined, appType: undefined }; currentPage.value = 1; loadData(); }
      ...
      <ElTable v-loading="loading" :data="data" border stripe>
        ...
        <ElTableColumn prop="appType" label="类型" min-width="80" align="center">
          <template #default="{ row }">
            <ElTag :type="row.appType === 'reward' ? 'success' : 'danger'">{{ row.appType === 'reward' ? '奖励' : '惩罚' }}</ElTag>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="category" label="类别" min-width="100">
          <template #default="{ row }">{{ getDictLabel(row.appType === 'reward' ? rewardCategoryOptions : punishCategoryOptions, row.category) }}</template>
        </ElTableColumn>
      ...
      <ElForm label-width="100px" :model="formData">
        ...
        <ElFormItem label="类别" required>
          <ElSelect v-model="formData.category" placeholder="请选择类别" style="width: 100%">
            <ElOption v-for="item in (formData.rewardType === 1 ? rewardCategoryOptions : punishCategoryOptions)" :key="item.dictValue" :label="item.dictLabel" :value="item.dictValue" />
          </ElSelect>
        </ElFormItem>
        <ElFormItem label="金额">
