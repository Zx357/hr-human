...
const formData = ref<Application & { employeeName?: string; employeeNo?: string; companyName?: string; deptName?: string }>({
  employeeId: undefined as any, appType: 'business'
});
    ...
    <ElCard class="table-card">
      <template #header>
        <div class="flex items-center justify-between">
          <span>出差申请列表</span>
          <ElButton type="primary" @click="handleAdd"><template #icon><icon-ep-plus /></template>发起出差申请</ElButton>
        </div>
      </template>
    ...
    <ElDialog v-model="dialogVisible" title="发起出差申请" width="600px" destroy-on-close>
      <ElForm label-width="100px" :model="formData">
        ...
        <ElFormItem label="出差小时">
          ...
          <div v-if="businessHours === 0 && formData.employeeId && dateRange" class="text-orange-500 text-12px mt-4px">提示：出差小时为0，可能是员工在该时间段没有排班</div>
        </ElFormItem>
    ...
    <ElDialog v-model="employeeDialogVisible" title="选择员工" width="900px" destroy-on-close append-to-body>
      ...
      <ElTable v-loading="employeeDialogLoading" :data="employeeDialogData" border stripe max-height="400px">
        ...
        <ElTableColumn label="操作" width="80" align="center" fixed="right">
          <template #default="{ row }"><ElButton type="primary" link size="small" @click="handleSelectEmployee(row)">选择</ElButton></template>
        </ElTableColumn>
      </ElTable>
      <div class="mt-16px flex justify-end">
