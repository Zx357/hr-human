...
const formData = ref<Application & { employeeName?: string; employeeNo?: string; companyName?: string; deptName?: string; entryDateDisplay?: string }>({
  employeeId: undefined as any, appType: 'regularization'
});
...
async function loadDictData() {
  const [empTypeRes, genderRes] = await Promise.all([
...
async function loadData() {
  loading.value = true;
  try {
    const res = await fetchApplicationPage({ pageNum: currentPage.value, pageSize: pageSize.value, appType: 'regularization', employeeName: searchParams.value.employeeName || undefined, employeeNo: searchParams.value.employeeNo || undefined, status: searchParams.value.status });
...
function handleAdd() {
  formData.value = { employeeId: undefined as any, appType: 'regularization', regularDate: '', probationEndDate: '', evaluation: '', newEmployeeType: '', reason: '', employeeName: '', employeeNo: '', companyName: '', deptName: '', entryDateDisplay: '' };
...
function handleSelectEmployee(row: Api.Hr.Employee) {
  ...
  if (row.entryDate) {
    ...
    formData.value.probationEndDate = entryDate.toISOString().split('T')[0];
  }
      ...
      <ElTable v-loading="loading" :data="data" border stripe>
        ...
        <ElTableColumn prop="newEmployeeType" label="转正后类型" min-width="100" align="center">
          <template #default="{ row }">
            {{ getDictLabel(employeeTypeOptions, row.newEmployeeType) }}
          </template>
        </ElTableColumn>
    ...
    <ElDialog v-model="dialogVisible" title="新增转正申请" width="600px" destroy-on-close>
      <ElForm label-width="100px" :model="formData">
        ...
        <ElFormItem label="转正日期" required>
          <ElDatePicker v-model="formData.regularDate" type="date" placeholder="选择转正日期" style="width: 100%" value-format="YYYY-MM-DD" />
        </ElFormItem>
